...
from stopit2 import ThreadingTimeout
from Xlib import X, Xatom, display
from Xlib.error import BadAtom, CatchError
...
class XGetter(Thread):
    ...
    def initX(self):
        ...
        self.window = self.display.screen().root.create_window(0, 0, 1, 1, 0, X.CopyFromParent)
    ...
    def processEvent(self, xevent):
        ...
        if target == "TARGETS":
            try:
                target_atoms = self.window.get_full_property(xevent.target, Xatom.ATOM).value
            ...
            try:
                prop = self.window.get_full_property(xevent.target, xevent.target)
            except Exception as e:
                ErrorReporter.print(e)
                return
            if prop:
...
class XSetter(Thread):
    ...
    def run(self):
        def serve():
            while True:
                ...
                if xevent is None:
                    ...
                    client_prop = process_request(xevent.requestor, xevent.property, xevent.target)
                    ...
                    xevent.requestor.send_event(selection_notify, onerror=errHandler)
                    try:
                        self.display.flush()
                    except Exception as e:
                        ErrorReporter.print(e)
                    ...
                    print("Failed to transfer ownership to Clipboard Manager", file=sys.stderr)
                    break
        ...
        def process_request(client, property, target):
            ...
            if target == self.TARGETS:
                ...
                try:
                    wanted_prop = client.get_full_property(client_prop, X.AnyPropertyType)
                except Exception as e:
                    ErrorReporter.print(e)
                    self.reset()
                    return
                if wanted_prop:
                    wanted = [wanted_prop.value[i : i + 2] for i in range(0, len(wanted_prop.value), 2)]
        ...
        while True:
            ...
            self.window.set_selection_owner(self.SELECTION, X.CurrentTime, onerror=errHandler)
            try:
                self.display.flush()
            except Exception as e:
                ErrorReporter.print(e)
            ...
            if current_owner == self.window:
                server = Thread(target=serve, name="klembord XSetter server", daemon=True)
                server.start()
    ...
    def store(self):
        if self.content_set:
            try:
                clipboardManager = self.display.get_selection_owner(self.CLIPBOARD_MANAGER)
            except BadAtom as e:
                ErrorReporter.print(e)
