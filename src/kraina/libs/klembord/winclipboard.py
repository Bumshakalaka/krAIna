...
TARGETS = {"CF_OEMTEXT": 7, "CF_PRIVATEFIRST": 0x0200, "CF_PRIVATELAST": 0x02FF, "CF_TEXT": 1, "CF_UNICODETEXT": 13}
NAMES = {
    1: "CF_TEXT",
    2: "CF_BITMAP",
    3: "CF_METAFILEPICT",
...
class WinClipboard(object):
    ...
    windll.user32.GetClipboardFormatNameW.argtypes = (c_uint, c_wchar * 1024, c_int)
    ...
    def get(self, targets):
        ...
        for target in targets:
            if target == "TARGETS":
                ...
                formats[target] = windll.user32.RegisterClipboardFormatW(create_unicode_buffer(target, 1024))
                ...
                if format < 0:
                    ...
                    while next_format:
                        if next_format in NAMES:
                            ...
                            if windll.user32.GetClipboardFormatNameW(next_format, name, 1024) > 0:
                                names.append(name.value)
                        next_format = windll.user32.EnumClipboardFormats(next_format)
    ...
    def set(self, content):
        ...
        for target, data in content.items():
            ...
            if target in UNSUPPORTED:
                ...
                if target in TARGETS:
                    ...
                    format = windll.user32.RegisterClipboardFormatW(create_unicode_buffer(target, 1024))
                    if isinstance(data, str):
        ...
        if windll.user32.OpenClipboard(None):
            ...
            for format, data in formats.items():
                handle = windll.kernel32.GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, len(data) + 2)
                ptr = windll.kernel32.GlobalLock(handle)
    ...
    def wrap_html(self, fragment_str):
        ...
        header = html_header.format(*[str(x).zfill(8) for x in sizes]).encode("utf8")
