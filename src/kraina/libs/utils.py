...
import kraina.libs.images as images  # noqa
...
def to_md(text: str, col: Optional[str] = None) -> str:
    ...
    def insert_img(m: re.Match) -> str:
        ...
        name = images.chat_images.create_from_url(m.group("img_data"), m.group("img_name"))
        width, height = images.chat_images.pil_image[name]["resized-150"].size
    ...
    def insert_mermaid(m: re.Match) -> str:
        ...
        if images.chat_images.get(name):
            width, height = images.chat_images.pil_image[name]["resized-600"].size
            return (
                f'<img src="{images.chat_images.get_file_url(name)}" alt="{name}" width="{width}" height="{height}"/>'
            )
        else:
    ...
    def insert_latex(latex_, idx_, inverted) -> Tuple[str, str]:
        ...
        if images.chat_images.get(name) and images.chat_images.get(name) != "broken":
            return f'<img src="{images.chat_images.get_file_url(name, inverted)}" alt="{name}"/>', idx_
        elif images.chat_images.get(name) != "broken":
            ...
            if ret.get("imageUrl"):
                ...
                images.chat_images[name] = "broken"
                return latex_, idx_
        else:
            return latex_, idx_
...
def find_assets(type: Literal["assistants", "snippets", "tools", "macros"], build_in: Path) -> List[Path]:
    """Return a list of available asset directories for the given type.

    Searches for directories matching the specified type (e.g., "assistants", "snippets", "tools", "macros")
    in the provided built-in path and in the script's parent directories or current working directory.
    If found, appends them to the result list.

    :param type: Asset type as a string ("assistants", "snippets", "tools", "macros").
    :param build_in: Path to the built-in asset directory.
    :return: List of Paths to found asset directories.
    """
    logger.debug(f"find_assets: {type}, {build_in}")
    set_ = [build_in]
    # Try to get the script's directory
    for script_dir in [Path(sys.argv[0]).parent, Path.cwd()]:
        script_type_dir = script_dir / type
        if script_type_dir.exists():
            set_.append(script_type_dir)
            logger.debug(f"Found {type} in {script_type_dir}")
            break
    else:
        logger.warning(f"No {type} found in {Path(sys.argv[0]).parent} or {Path.cwd().resolve()}")
    return set_
...
def _convert_data_url_to_file_url(m: re.Match) -> str:
    ...
    """
    name = images.chat_images.create_from_url(m.group("img_data"), m.group("img_name"), False)
    return f"![{m.group('img_name')}]({images.chat_images.get_file_uri(name)})"
