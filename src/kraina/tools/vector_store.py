...
from kraina.libs.llm import embedding, map_model
from kraina.libs.paths import STORE_PATH
from kraina.libs.utils import str_shortening
from kraina.tools.base import logger
from kraina.tools.vector_store_file_splitter import get_splitter
...
def vector_search(query: str, file_path: str, k: int = 10, model: str = "embed", force_api: str | None = None):
    ...
    if (STORE_PATH / f"{store_file_name}.pkl").exists():
        ...
        with open(STORE_PATH / f"{store_file_name}.pkl", "rb") as fd:
        ...
        with open(STORE_PATH / f"{store_file_name}.pkl", "wb") as fd:
